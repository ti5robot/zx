...
int32_t sendSimpleCanCommand(uint8_t numOfActuator, uint8_t *canIdList, int Command)
{
    ...
    torque_state.header.stamp = ros::Time::now();
    torque_state.name ={"A","B","C","D","E","F","G"};//name数组的第一个元素赋值
    torque_state.position.resize(7);
        ...
        if (VCI_Transmit(VCI_USBCAN2, 0, 0, send, 1) == 1)
        {
            ...
            if ((reclen = VCI_Receive(VCI_USBCAN2, 0, ind, rec, 3000, 100)) > 0) // 调用接收函数，如果有数据，进行数据处理显示。
            {
                ...
                // for(j=0;j<2;j++)
                {
                    ...
                    //std::cout << "ID:"<< j+1<< "    Decimal: " << decimal << std::endl;
...
void Listener::callback(const moveit_msgs::DisplayTrajectory::ConstPtr &msg)
{
    ...
    for (int i =0; i < n - 1; i++)
    {
        ...
        ori_position[6] = msg->trajectory[0].joint_trajectory.points[i].positions[6] / 6.28 * 101 * 65536;
        ...
        next_position[6] = msg->trajectory[0].joint_trajectory.points[i + 1].positions[6] / 6.28 * 101 * 65536;
        ...
        cn[6] = next_position[6] - ori_position[6];
        ...
        for (int i = 0; i < round_i; i++)
        {
            ...
            if (cn[6] < 0)
            {
                r7[i] = ori_position[6] - i * abs(int(cn[6] / round_i));
            }
            ...
            parameterlist2[6] = r7[i];

            // std::cout<<r1[i]<<std::endl;

            SCanInterface *canInterface = SCanInterface::getInstance();
            uint8_t canidlist2[10] = {1, 2, 3, 4, 5, 6,7}, cmd2[10] = {30,30, 30, 30, 30, 30, 30};
            sendCanCommand(7, canidlist2, cmd2, parameterlist2);
            sendSimpleCanCommand(7, canidlist2, 8);
